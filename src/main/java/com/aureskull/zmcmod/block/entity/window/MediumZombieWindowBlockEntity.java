...
import com.aureskull.zmcmod.block.custom.window.MediumZombieWindowBlock;
...
public class MediumZombieWindowBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory, ILinkable {
    ...
    public MediumZombieWindowBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.MEDIUM_ZOMBIE_WINDOW_BLOCK_ENTITY, pos, state);
    }
    ...
    public Text getDisplayName() {
        return Text.literal("Medium Zombie Doorway");
    }

    @Override
    protected void writeNbt(NbtCompound nbt){
        nbt.putInt("medium_zombie_doorway.plank", plank);
        if (linkedSpawnerPos != null) {
            nbt.put("medium_zombie_doorway.linked_spawner", NbtHelper.fromBlockPos(linkedSpawnerPos));
        }

        if (linkedZonePos != null) {
            nbt.put("medium_zombie_doorway.linked_zone", NbtHelper.fromBlockPos(linkedZonePos));
        }
        super.writeNbt(nbt);
    }
    ...
    public void readNbt(NbtCompound nbt){
        ...
        if (nbt.contains("medium_zombie_doorway.plank"))
            this.plank = nbt.getInt("medium_zombie_doorway.plank");

        if (nbt.contains("medium_zombie_doorway.linked_spawner")) {
            this.linkedSpawnerPos = NbtHelper.toBlockPos(nbt.getCompound("medium_zombie_doorway.linked_spawner"));
        }

        if (nbt.contains("medium_zombie_doorway.linked_zone")) {
            linkedZonePos = NbtHelper.toBlockPos(nbt.getCompound("medium_zombie_doorway.linked_zone"));
        }
    }
    ...
    public void tick(World world, BlockPos pos, BlockState state) {
        if(world.isClient()) {
            try{
                ...
                if(plank < MAX_PLANK){
                    Direction facing = state.get(MediumZombieWindowBlock.FACING);
                ...
                ZMCMod.LOGGER.error("An error occurred in the MediumZombieWindowBlockEntity tick method :" + e.getMessage() + e.getStackTrace());
            }
        }else{
    ...
    public void rebuild(ServerPlayerEntity player){
        if(plank < MAX_PLANK){
            ...
            assert world != null;
            if (!world.isClient) {
                world.setBlockState(pos, world.getBlockState(pos).with(MediumZombieWindowBlock.PLANKS, plank), 3);
    ...
    public void resetPlank() {
        ...
        if(world != null) world.setBlockState(pos, world.getBlockState(pos).with(MediumZombieWindowBlock.PLANKS, plank), 3);
    }
